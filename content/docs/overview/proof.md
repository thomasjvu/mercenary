...
`/verification?raidId=<raidId>&token=<raidAccessToken>` (canonical; `/receipt` redirects here)
...
- **Host TEE quote** — `GET /v1/host/attestation` (`teeVerified` / `verified`); structural verify by default, Phala Cloud when `BOSSRAID_HOST_TEE_SKIP_CLOUD_VERIFY` is unset
- **Signed runtime envelope** — `signedRuntime` on host attestation or `GET /v1/attested-runtime` when `MNEMONIC` is configured (does not imply TEE hardware verification)
- **Upstream marketplace TEE** — `POST /v1/marketplace/tee/attestation` for hosted seller quotes
- **Inference receipt verify** — `GET /v1/inference/receipts/:receiptId/verify`
