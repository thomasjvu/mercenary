...
| `BOSSRAID_RPC_URL`, `BOSSRAID_CHAIN_ID`                                                                            | Robinhood: mainnet `4663` / RPC `https://rpc.mainnet.chain.robinhood.com`                   |
| `BOSSRAID_REGISTRY_ADDRESS`, `BOSSRAID_ESCROW_ADDRESS`, `BOSSRAID_BOUNTY_ESCROW_ADDRESS`, `BOSSRAID_TOKEN_ADDRESS` | Contracts; token = USDG `0x5fc5…d168` on mainnet                                            |
| `BOSSRAID_CLIENT_PRIVATE_KEY`                                                                                      | Client/operator signer (raid + bounty onBehalf relayer); fund with ETH + USDG on Robinhood  |
...
| `BOSSRAID_HARNESS_IMAGE_ALLOWLIST` | Comma digests allowed for specialized agent images |
| `BOSSRAID_HARNESS_REQUIRE_IMAGE_ALLOWLIST` | `1` = fail specialized without allowlisted digest |
