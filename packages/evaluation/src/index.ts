...
export async function evaluateSubmission(
  ...
  if (!patchApply.ok) {
    return invalid(
      'patch_apply_failed',
      patchApply.error ?? 'Patch did not map cleanly onto the provided files.'
    );
  }
    ...
    const runtimeProbes = isPatchTask
      ...
      : {
          build: {
            passed: true,
            score: 1,
            summary: 'No build probe required for non-patch output.',
          },
          tests: {
            passed: 0,
            failed: 0,
            score: 0,
            summary: 'No runtime tests for non-patch output.',
          },
        };
