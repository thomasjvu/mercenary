#!/usr/bin/env node
/**
 * Run Boss Raid development smokes for all four agents.
 *
...
const evidenceJsonl =
  ...
  join(evidenceDir, `bossraid-development-smoke-${day}.jsonl`);
...
function reseedQuests() {
  ...
  process.stdout.write('Reseeding bossraid-development quests...\n');
  const result = spawnSync(
    'bash',
    [
      '-lc',
      `cd "${partyQuestDir}" && set -a && source .env.self-hosted && set +a && npx convex run seed:seedBossraidDevelopment`,
    ],
    { encoding: 'utf8' },
  );
  if (result.status !== 0) {
