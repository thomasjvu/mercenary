...
const apiPort = Number(process.env.PORT ?? String(8800 + (Date.now() % 1000)));
...
const sqliteFile =
  explicitSqliteFile && explicitSqliteFile !== "./temp/bossraid-state.sqlite"
    ? explicitSqliteFile
    : `./temp/private-game-raid-e2e-${Date.now()}.sqlite`;
const env = {
  ...process.env,
  PORT: String(apiPort),
...
try {
  ...
  const spawnResponse = await fetch(new URL("/v1/raid", apiBase), {
    ...
    body: await readFixture("./examples/game-raid/private-native-raid.json"),
  });
  if (!spawnResponse.ok) {
...
function verifyResult(result) {
  ...
  if (routingProof.policy?.privacyMode !== "strict") {
    throw new Error(`Expected strict privacy mode, received ${routingProof.policy?.privacyMode ?? "missing"}`);
  }
  ...
  for (const provider of primaryProviders) {
    ...
    if (!provider.erc8004Registered) {
      throw new Error(`Expected ERC-8004-registered primary provider, received ${provider.providerId}`);
    }
