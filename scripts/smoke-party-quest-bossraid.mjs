 ...
 *   AGENT — bossraid-code | bossraid-debug | bossraid-marketing | bossraid-research
 *   CAMPAIGN_SLUG — default bossraid-development
 *   BOSSRAID_REPO — path to repo (default ~/bossraid-ops/mercenary)
...
const AGENT_PROFILES = {
  'bossraid-code': {
    ...
    defaultQuest: 'Daily check on development',
    workflowPath: 'pnpm check',
  },
  'bossraid-debug': {
...
async function main() {
  ...
  const agentId = process.env.AGENT?.trim() || 'bossraid-code';
  ...
  const agentsJson =
    ...
    join(homedir(), 'bossraid-ops/phantasy-agent/.phantasy/party-quest/agents.json');
  ...
  const campaignSlug = process.env.CAMPAIGN_SLUG?.trim() || 'bossraid-development';
  const repoPath = resolve(
    process.env.BOSSRAID_REPO?.trim() || join(homedir(), 'bossraid-ops/mercenary'),
  );
  ...
  const evidence = {
    schemaVersion: 'bossraid.party-quest.development-smoke.v1',
...
function writeEvidence(evidence) {
  ...
  const appendPath =
    ...
    join(outDir, `bossraid-development-smoke-${day}.jsonl`);
  appendFileSync(appendPath, `${JSON.stringify(evidence)}\n`);
  writeFileSync(join(outDir, `bossraid-development-smoke-${day}.json`), `${JSON.stringify(evidence, null, 2)}\n`);
