...
export async function ensureInfisicalSecretPath({ domain, projectId, envName, secretPath }) {
  ...
  for (const segment of segments) {
    const response = await fetch(`${domain}/api/v1/folders`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    if (!response.ok && response.status !== 409) {
      ...
      if (!body.includes('already exists')) {
        throw new Error(
          `Infisical folder create failed for ${currentPath}${segment}: ${response.status} ${body}`
        );
      }
    }
