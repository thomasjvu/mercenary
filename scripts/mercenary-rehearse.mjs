...
const rehearsalEnv = {
  ...
  BOSSRAID_X402_RAID_SURCHARGE_USD: process.env.BOSSRAID_X402_RAID_SURCHARGE_USD ?? '0.01',
  BOSSRAID_X402_CHAT_SURCHARGE_USD: process.env.BOSSRAID_X402_CHAT_SURCHARGE_USD ?? '0.002',
...
try {
  if (!rehearsalEnv.BOSSRAID_MODEL_API_KEY || !rehearsalEnv.BOSSRAID_MODEL) {
    throw new Error(
      'BOSSRAID_MODEL_API_KEY and BOSSRAID_MODEL are required for pnpm mercenary:rehearse because the local providers must become ready.'
    );
  }
  ...
  console.log(JSON.stringify({ step: 'x402_raid_wallet' }, null, 2));
  await runCommand('pnpm', ['test:x402:e2e', '--', '--mode', 'wallet', '--route', 'raid']);

  console.log(JSON.stringify({ step: 'x402_chat_wallet' }, null, 2));
  await runCommand('pnpm', ['test:x402:e2e', '--', '--mode', 'wallet', '--route', 'chat']);

  console.log(JSON.stringify({ step: 'mcp_delegate' }, null, 2));
