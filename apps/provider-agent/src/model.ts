...
function normalizeSubmission(result: unknown, task: ProviderTaskPackage): ModelSubmission {
  ...
  if (wantsPatch) {
    if (
      typeof candidate.patchUnifiedDiff !== 'string' ||
      candidate.patchUnifiedDiff.trim().length === 0
    ) {
  ...
  const diffFiles =
    typeof candidate.patchUnifiedDiff === 'string'
      ? parseTouchedFilesFromDiff(candidate.patchUnifiedDiff)
      : [];
  const filesTouched = [...new Set([...parsedFiles, ...diffFiles])].filter((file) =>
    knownFiles.has(file)
  );
  ...
  return attachContributionRole(
    {
      ...
      confidence,
      claimedRootCause:
        typeof candidate.claimedRootCause === 'string' ? candidate.claimedRootCause : undefined,
      ...
      const veniceSubmission = await generateStructuredWithVenice<ModelSubmission>(
        {
          ...
          timeoutMs: Math.min(timeoutMs, TIMEOUTS.VENICE_TIMEOUT),
        },
        {
          systemPrompt: [
