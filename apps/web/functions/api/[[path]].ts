...
export async function onRequest(context: PagesContext): Promise<Response> {
  ...
  const upstreamUrl = new URL(
    buildRelativePath(context.params.path, requestUrl.search),
    upstreamBase
  );
...
async function rewriteX402PaymentRequiredResponse(
  response: Response,
  requestUrl: URL
): Promise<Response> {
...
function normalizePaymentRequired(
  ...
  return {
    ...
    accepts: paymentRequired.accepts?.map((accept) => ({
      ...
      network: normalizeX402Network(
        typeof accept.network === 'string' ? accept.network : undefined
      ),
