...
const NATIVE_CHAT_SYSTEM_PROMPT =
  'You are Mercenary on Boss Raid. Reply directly and concisely. If the user is greeting you or asking what you can do, answer conversationally and do not pretend a build already happened. Only treat the request as a real seeded GB Studio build when the user explicitly asks for that game package.';

function buildGbStudioSeedFiles(normalizedBrief: string) {
  return [
    {
  ...
  "notes": "One-room microgame scaffold for Mercenary seeded GB Studio raid."
}
`,
      sha256: 'seed-gbstudio-project',
    },
    {
...
- Timer and key HUD icons
`,
      sha256: 'seed-creative-brief',
    },
  ];
}

function buildSeededGameRaidPayload(normalizedBrief: string) {
  return {
    agent: 'mercenary-v1',
    taskType: 'game_build',
    task: {
      ...
      language: 'typescript',
      framework: 'gb-studio',
      files: buildGbStudioSeedFiles(normalizedBrief),
      failingSignals: {
        errors: [],
        reproSteps: [
...
function buildNativeChatRaidPayload(normalizedBrief: string) {
  ...
  return {
    ...
    task: {
      ...
      failingSignals: {
        ...
        expectedBehavior:
          'Return one clean Mercenary answer. Do not claim a build or artifact package exists unless the user explicitly asked for the seeded GB Studio raid build.',
      },
    },
    output: {
      primaryType: 'text',
...
export { buildGbStudioSeedFiles, buildSeededGameRaidPayload, buildNativeChatRaidPayload };
