...
test('summarizeRaidReceipt preserves ERC-8004 verification and ERC-8183 lifecycle proof state', () => {
  ...
  const result: BossRaidResultOutput = {
    ...
    settlementExecution: {
      ...
      jobIds: ['7'],
      warnings: [
        'provider-alpha: successful child job is submitted but still awaiting evaluator completion.',
      ],
      allocations: [
        {
          providerId: 'provider-alpha',
  ...
  assert.equal(
    receipt.routingProof?.providers[0]?.erc8004VerificationCheckedAt,
  ...
  assert.equal(
    receipt.settlementExecution?.warnings?.[0]?.includes('awaiting evaluator completion'),
    true
  );
  assert.equal(receipt.settlementExecution?.childJobs[0]?.requestedAction, 'complete');
  assert.equal(
    receipt.settlementExecution?.childJobs[0]?.nextAction,
    'Evaluator completion is still required.'
  );
});
