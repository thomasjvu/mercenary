...
export function upsertSellerVeniceConfig(
  ctx: ControlStateContext,
  input: {
    wallet: string;
    apiKey: string;
  ...
  return upsertSellerUpstreamConfig(ctx, { ...input, provider: 'venice' }, nowMs);
}

export function readSellerVeniceConfig(
  ctx: ControlStateContext,
  wallet: string,
  nowMs = Date.now()
): SellerUpstreamConfigEntry | undefined {
  return readSellerUpstreamConfig(ctx, wallet, 'venice', nowMs);
}

export function readSellerVeniceApiKey(
  ctx: ControlStateContext,
  wallet: string,
  cipher: SecretCipher,
  nowMs = Date.now()
): string | undefined {
  return readSellerUpstreamApiKey(ctx, wallet, 'venice', cipher, nowMs);
}

export function deleteSellerVeniceConfig(
  ctx: ControlStateContext,
  wallet: string,
  nowMs = Date.now()
): boolean {
  return deleteSellerUpstreamConfig(ctx, wallet, 'venice', nowMs);
}

export const sanitizeSellerVeniceConfig = sanitizeSellerUpstreamConfig;
