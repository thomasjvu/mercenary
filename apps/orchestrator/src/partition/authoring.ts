...
import { type ContributionWorkstreamTemplate, type TaskPlanningContext } from './types.js';

export function expandRoleTemplates(
...
export function authorContributionFamilyWorkstreams(
...
export function taskCanRouteThroughGameWorkstreams(task: SanitizedTaskSpec): boolean {
...
export function isGameTask(task: SanitizedTaskSpec): boolean {
  const haystack = [
    task.framework,
    task.taskTitle,
