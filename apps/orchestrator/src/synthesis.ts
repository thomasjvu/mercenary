...
function buildWorkstream(
  ...
  const base =
    workstreamPrimaryType === 'patch'
      ? (group.entries.find((item) => hasPatch(item.submission.patchUnifiedDiff)) ??
        group.entries[0]!)
      : group.entries[0]!;
  ...
  return {
    ...
    patchUnifiedDiff:
      workstreamPrimaryType === 'patch' ? base.submission.patchUnifiedDiff : undefined,
    artifacts,
  };
}
...
function buildCanonicalTextAnswer(
  ...
  const groupedIds = new Set(
    [constraint?.id, risk?.id, execution?.id].filter((value): value is string => Boolean(value))
  );
...
function selectBaseWorkstream(
  ...
  const preferred =
    primaryType === 'patch'
      ? workstreams.find(
          (item) => matchesWorkstreamGroup(item.id, 'implementation') && item.patchUnifiedDiff
        )
      : primaryType === 'text' || primaryType === 'json'
        ? workstreams.find((item) => matchesWorkstreamGroup(item.id, 'answer'))
        : workstreams.find(
            (item) => item.primaryType === primaryType && (item.artifacts?.length ?? 0) > 0
          );
...
function selectArtifactPrimaryType(entries: RankedSubmission[]): OutputType | undefined {
  ...
  for (const type of artifactOrder) {
    if (
      entries.some((item) =>
        (item.submission.artifacts ?? []).some((artifact) => artifact.outputType === type)
      )
    ) {
      return type;
    }
...
function compareWorkstreams(
  ...
  const order: readonly string[] =
    primaryType === 'patch' ? PATCH_WORKSTREAM_ORDER : TEXT_WORKSTREAM_ORDER;
...
function getWorkstreamMetadata(
  entry: RankedSubmission
): Pick<WorkstreamGroup, 'id' | 'label' | 'objective'> {
...
function collectSupportingSignals(
  base: RankedSubmission,
  supportingEntries: RankedSubmission[]
): string[] {
