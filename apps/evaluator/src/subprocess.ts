...
export function readWorkerIsolationLabel(
  env: NodeJS.ProcessEnv = process.env
): 'per_job_process' | 'per_job_container' {
...
async function executeRuntimeProbeInSubprocess(
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('error', (error) => {
      ...
      reject(new RuntimeProbeExecutionError(500, 'sandbox_worker_spawn_failed', error.message));
    });
    child.on('close', (code, signal) => {
      ...
      if (timedOut) {
        reject(
          new RuntimeProbeExecutionError(
            504,
            'sandbox_timeout',
...
async function executeRuntimeProbeInContainer(
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('error', (error) => {
      ...
      reject(new RuntimeProbeExecutionError(503, 'sandbox_launcher_unavailable', error.message));
    });
...
function resolveWorkerArgs(currentModuleUrl: string): string[] {
  ...
  const workerPath = fileURLToPath(
    new URL(useTsx ? './job-worker.ts' : './job-worker.js', currentModuleUrl)
  );
...
function buildDockerRunArgs(
  ...
  return [
    ...
    String(
      readPositiveInteger(
        env.BOSSRAID_EVAL_JOB_CONTAINER_PIDS_LIMIT,
        DEFAULT_JOB_CONTAINER_PIDS_LIMIT
      )
    ),
    '--user',
    '10001:10001',
...
async function removeContainer(
  ...
  await new Promise<void>((resolve) => {
    const child = spawn('docker', ['--host', `unix://${socketPath}`, 'rm', '-f', containerName], {
      cwd: process.cwd(),
      stdio: 'ignore',
      env: buildWorkerEnv(env),
    });
...
function summarizeWorkerFailure(
  code: number | null,
