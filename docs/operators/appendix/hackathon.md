...
| MetaMask Cookoff: x402 + ERC-7710   | `/demo` paid raid mode uses MetaMask Smart Accounts Kit with ERC-7710 delegation payments against the MetaMask facilitator.          | `packages/smart-pay`, `BOSSRAID_X402_ASSET_TRANSFER_METHOD=erc7710`, `POST /v1/raid`                                            | Strong when Base mainnet facilitator + funded wallet are configured.                                                                   |
| MetaMask Cookoff: Best Agent        | Mercenary remains the main orchestrator while the browser flow connects, grants ERC-7715 budget, and pays for raids.                 | `/demo`, `/account`, `GET /v1/agent.json`, `GET /v1/raid/:id/agent_log.json`                                                    | Strong now for demo flow shape.                                                                                                        |
