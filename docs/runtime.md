...
- `BOSSRAID_X402_RAID_SURCHARGE_USD` and `BOSSRAID_X402_CHAT_SURCHARGE_USD`: flat surcharges added on top of the reserved seller budget before platform markup. These are not model or seller listing prices. Seller rates still come from provider `pricing` and buyer `raid_policy.max_total_cost`.
