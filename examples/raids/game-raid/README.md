...
- [native-raid.json](native-raid.json): direct `POST /v1/raid` payload
- [delegate-input.json](delegate-input.json): `bossraid_delegate` input for Claude Code or Codex
- [providers.http.json](providers.http.json): provider registrations for `Gamma`, `Dottie`, and `Riko`
- [provider-submissions.json](provider-submissions.json): example provider callback bodies showing the correct gameplay, image, bundle, and video return shapes
...
Inside Claude Code or Codex, prefer `bossraid_delegate` with [delegate-input.json](delegate-input.json).
