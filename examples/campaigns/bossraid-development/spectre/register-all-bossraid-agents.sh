...
BOSSRAID_REPO="${BOSSRAID_REPO:-$HOME/bossraid-ops/mercenary}"
AGENT_ROOT="${PHANTASY_AGENT_ROOT:-$HOME/bossraid-ops/phantasy-agent}"
SEED_EVIDENCE="${BOSSRAID_RUNTIME_SEED_FILE:-$HOME/bossraid-ops/runtime-agent-seed.json}"
ENV_DIR="${BOSSRAID_AGENT_ENV_DIR:-$HOME/bossraid-ops/workspaces/env}"

BOSSRAID_AGENTS=(
  "bossraid-marketing:bossraid-phantasy-agent"
  "bossraid-research:bossraid-hermes-agent"
  "bossraid-debug:bossraid-openclaw-agent"
  "bossraid-code:bossraid-opencode-agent"
)

PORT_MAP=(
  "bossraid-phantasy-agent:2200"
  "bossraid-hermes-agent:2201"
  "bossraid-openclaw-agent:2202"
  "bossraid-opencode-agent:2203"
)
...
for entry in "${BOSSRAID_AGENTS[@]}"; do
  agent="${entry%%:*}"
  framework_id="${entry##*:}"
  ...
  if [ ! -f "${config}" ]; then
    src="${BOSSRAID_REPO}/examples/campaigns/bossraid-development/phantasy-agent-configs/${agent}.json"
    if [ -f "${src}" ]; then
