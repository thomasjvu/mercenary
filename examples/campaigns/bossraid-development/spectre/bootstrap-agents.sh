...
#   bash ~/bossraid-ops/mercenary/examples/campaigns/bossraid-development/spectre/bootstrap-agents.sh
...
TEMPLATE_ROOT="${BOSSRAID_REPO}/examples/campaigns/bossraid-development/workspaces"
...
cp "${BOSSRAID_REPO}/examples/campaigns/bossraid-development/phantasy-agent-configs/"*.json \
...
if [ -n "${FORGEJO_TOKEN:-}" ]; then
  FORGEJO_TOKEN="${FORGEJO_TOKEN}" WORKSPACE_ENV_DIR="${WORKSPACES}/env" \
    node "${BOSSRAID_REPO}/examples/campaigns/bossraid-development/scripts/setup-forgejo-agent-users.mjs"
else
...
BOSSRAID_LOG_DIR="${BOSSRAID_OPS}/logs" \
  bash "${BOSSRAID_REPO}/examples/campaigns/bossraid-development/spectre/start-agents.sh"
...
BOSSRAID_RUNTIME_SEED_FILE="${SEED_FILE}" \
  bash "${BOSSRAID_REPO}/examples/campaigns/bossraid-development/spectre/register-all-bossraid-agents.sh"

echo "Done. Dogfood: node ${BOSSRAID_REPO}/examples/campaigns/bossraid-development/scripts/dogfood-party-quest-bossraid.mjs --pause-bridges --reseed"